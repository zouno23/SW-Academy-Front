...
try {
    ...
    const response:AxiosResponseType =  await axios.get<AxiosResponseType>(`${url}/quiz`
...
export const GetScores = async ()=>{
    try {
        const jwt = GetJWT()
        if (!jwt) throw new Error("No JWT available");
        const response:AxiosResponseType =  await axios.get<AxiosResponseType>(`${url}/quizScores`
        ,  {headers:{Authorization:`Bearer ${jwt}`}}
        )
        console.log(response)
        return {error:null , response:response.data}
