...
import { GetProducts, TeacherBestCourses } from "@/app/Actions/DashboardActions";
...
async function CourseList () {
    ...
    <div className=" flex w-full justify-between p-8">
        <h3 className="text-3xl font-bold">{role==="Teacher"?"Best" : "My"} Courses</h3>
        <Link href="/Student/courses"><Button>View all courses</Button></Link>
    </div>
    <div className="overflow-x-auto w-full  flex-col flex px-8 py-2">
    <ListHeader role={role}/>
    {list.length ===0 ?<NoCourses/>:list.map((course:any,item:number)=> <Course props={course} key={item} role={role}/> )}
    </div>
    </div> );
}
