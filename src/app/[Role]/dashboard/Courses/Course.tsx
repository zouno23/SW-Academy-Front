...
function Course( {props, role}:any) {
    ...
    <div className="h-full flex flex-col justify-center gap-2 w-2/3">
        <h4 className="font-semibold">{role === "Teacher"?props?.Title:props?.Course.Title}</h4>
        {
          role ==="Teacher"?
    ...
    <div className="basis-1/3 flex justify-center"> {role ==="Teacher"?"Price":props?.IsCompleted?<Button disabled>Completed</Button>:<Button>{props.Progress > 0? "Continue":"Start"}</Button> } </div>
    <div className="basis-1/3 flex justify-center"> {role ==="Teacher"?"Number of Sellings":<StarsRating Rating={props?.Course.Rating} key="rating" />}</div>
    <div className="basis-1/3 flex justify-center"> {role ==="Teacher"?props?.Lessons?.length:props?.Course.Lessons.length}</div>
    </div>
</div>
  );
}
