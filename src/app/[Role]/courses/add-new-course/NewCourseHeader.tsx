...
function NewCourseHeader({searchParams}:
    ...
    const NewCourseProgress = (page : string, index: number , pageNumber:string) => {
        return  (

            <span className={cn("flex gap-2 items-center w-fit  justify-start bg-white z-10 px-4")}>                 
                    <Link href={`?step=${index+1}`} className={cn("bg-gray-200 rounded-full p-2 size-10  text-center", index+1 == parseInt(pageNumber) && "bg-blue-600  text-white")}>{index+1}</Link>
...
<div className="flex flex-col  w-full  justify-between items-center bg-blue-600 border rounded-xl divide-y-2 dark:bg-slate-900 ">
    <div  className="flex w-full  justify-between items-center py-8 px-8">
        <div className="w-1/2 flex justify-center flex-col gap-2 text-white">
            ...
            <h4 className="text-lg font-semibold">Just fill the form and create your course.</h4>
        </div>
    ...
    <div className="flex w-full py-6 justify-between px-8 relative overflow-hidden bg-white rounded-lg border">
        {
            pages.map((page , index)=>NewCourseProgress(page,index , step || "1") )
        }
        <Separator  decorative className="w-11/12 absolute top-1/2 bg-gray-300"/>
    </div>
