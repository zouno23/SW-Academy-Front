...
async function CourseList({
  ...
  else ListType = searchParams.type || "All";
  ...
  if (list && searchParams.query) {
    subList = list.filter((course: any) => {
      ...
      if (ListType === "Owned" && role != "Teacher") {
        return (
  ...
  const ListMapping = () => {
    return subList.map((course: any, item: number) =>
      item < 10 * parseInt(ListPage) &&
      item > 10 * (parseInt(ListPage) - 1) - 1 ? (
        <Course
          props={course}
  ...
  return (
    <div className=" bg-white border w-full h-max  flex flex-col rounded-xl divide-y-2 dark:bg-slate-900 ">
        ...
        {role === "Student" ? (
          <Popover>
            <PopoverTrigger asChild>
              <Button
                variant="ghost"
                className="border p-4 py-8 text-3xl font-bold gap-3"
              >
            ...
            <PopoverContent className="flex flex-col gap-2 p-0 w-max ">
              <Link
                href={
                  `?type=${ListType === "All" ? "Owned" : "All"}` +
                  OldSearchParams
                }
              >
                <Button
                  variant="ghost"
                  className=" p-14 py-6  text-2xl font-bold justify-start gap-3"
                >
                  {ListType === "All" ? "My Courses" : "All Courses"}
                </Button>
              </Link>
            </PopoverContent>
          </Popover>
        ...
        <div className="flex gap-4 w-2/5 max-md:w-full max-md:justify-center  justify-end">
          ...
          {role === "Teacher" && (
            <Link href="/Teacher/courses/add-new-course">
              <Button className="inline-flex w-auto h-auto">
                {" "}
                Create new Course
              </Button>
            </Link>
      ...
      {subList.length > 0 && (
        <ListPagination searchParams={searchParams} max={subList.length / 10} />
      )}
    </div>
  );
}
