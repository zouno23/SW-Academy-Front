...
function Course( {props, role , type}:any) {
    ...
    <div className="h-full flex flex-col justify-center gap-2 w-2/3">
        <h4 className="font-semibold">{role === "Teacher" || type ==="All" ?props?.Title:props?.Course?.Title}</h4>
        {
        ...
        <span className=" text-end">
            {props.Progress + "%"}
            <Progress value={parseInt(props.Progress)} className="mt-1 border-blue-600"/>
        </span>
        }
    </div>
    </div>
    <div className=" flex w-1/2 px-6 flex-row-reverse text-center justify-between ">
    <div className="basis-1/3 flex justify-center"> {role ==="Teacher" || type ==="All"  ?props?.Price||"Free":props?.IsCompleted?<Button disabled>Completed</Button>:<Button>{props.Progress > 0? "Continue":"Start"}</Button> } </div>
    <div className="basis-1/3 flex justify-center"> {role ==="Teacher" || type ==="All"  ?props?.sellings||0:<StarsRating Rating={props?.Course?.Rating} key ="rating" />}</div>
    <div className="basis-1/3 flex justify-center"> {role ==="Teacher" || type ==="All"  ?props?.Lessons?.length:props?.Course?.Lessons?.length}</div>
    </div>
</div>
  );
}
