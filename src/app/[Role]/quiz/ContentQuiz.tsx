    ...
    <main className=" bg-white  rounded-xl flex flex-col overflow-auto h-full place-items-center gap-8 p-4 sm:p-6 md:p-8">
      <div className="w-full max-w-xl space-y-2 ">
        ...
        <p className="text-gray-500 dark:text-gray-400">
          Test your knowledge with our daily quiz. Select the correct answer for each question.
        </p>
      </div>
      ...
      {loading && <Loading />}
        ...
        {!available &&!hasAttemptedQuiz && ( // only show the message if hasAttemptedQuiz is false
          <p className="text-gray-500 dark:text-gray-400">
          ...
          <Card>
            <CardContent className="flex flex-col items-center justify-center gap-2 p-6">
              ...
              <div className="text-2xl font-bold">
                    {Data.lastTwoScores.length > 0 ? (
                        Data.lastTwoScores[0]
                    ) : (
                        "-"
                    )}
                </div>
              <div className="text-sm text-gray-500 dark:text-gray-400">Most recent</div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="flex flex-col items-center justify-center gap-2 p-6">
              <TrophyIcon className="h-8 w-8 text-gray-500 dark:text-gray-400" />
              <div className="text-2xl font-bold">
                    {Data.lastTwoScores.length > 1 ? (
                        Data.lastTwoScores[1]
                    ) : (
                        "-"
                    )}
                </div>
              <div className="text-sm text-gray-500 dark:text-gray-400">Second most recent</div>
            </CardContent>
          </Card>
