...
export default function NavigationButtons() {
    ...
    return (
      <Button
        className={cn(
          ...
          item.page.toLocaleLowerCase().replace(" ", "").localeCompare(state) ==
            0 &&
  ...
  useEffect(() => {
    ...
    if (state === ("courses" || "bootcamps")) setIsHere(true);
        ...
        item.page === "Products" ? (
          <Accordion type="single" collapsible className="" key={index}>
            <AccordionItem value="item-1">
              <AccordionTrigger
                ...
                type={undefined}
                className={cn(
                  "h-10 font-semibold items-center flex w-full gap-3 justify-start p-2 bg-white text-gray-400 hover:bg-blue-600/65 hover:text-white dark:bg-transparent dark:text-gray-400 dark:hover:bg-blue-600/65 dark:hover:text-white rounded-md ",
                  isHere &&
                    "bg-blue-600/65 text-white  dark:bg-blue-600/65 dark:text-white rounded-md"
                )}
              >
                {item.icon}
              ...
              <AccordionContent className="ml-4 mt-1">
                <Link
                  href={`/${pathname.split("/")[1] + "/"}courses`}
                  key={index}
                >
                  {routingButton(
                    { page: "Courses", icon: <Notebook className="size-5" /> },
                    "Courses"
                  )}
                </Link>
                <Link
                  href={`/${pathname.split("/")[1] + "/"}bootcamps`}
                  key={index}
                >
                  {routingButton(
                    {
          ...
          <Link
            href={`/${
              pathname.split("/")[1] +
              "/" +
              item.page.toLocaleLowerCase().replace(" ", "")
            }`}
            key={index}
          >
