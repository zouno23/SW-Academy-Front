...
const ChatBotContent = () => {
  const { messages, inputValue, setInputValue, sendMessage, handleKeyDown } =
    useChatbot();
    ...
    <div className="flex flex-col items-center justify-center h-screen px-5  bg-gray-100 dark:bg-gray-900">
      <div className="bg-white dark:bg-gray-800 shadow-lg rounded-lg w-full  m-10 p-6">
        ...
        <div
          ...
          className=" flex flex-col h-[500px] p-3 bg-gray-100 dark:bg-gray-850 rounded-lg overflow-y-auto"
        >
          {messages.map((message, index) => (
            ...
            >
              <p className="text-xs text-gray-600 dark:text-gray-400 ml-2 mr-2">
                {message.type === "user" ? "User" : "IA"}
              </p>
              <div
                className={`bg-${
                  message.type === "user" ? "blue-500" : "gray-200"
                } dark:bg-${
                  message.type === "user" ? "gray-700" : "gray-700"
                } rounded-lg p-3 `}
              >
                <p
                  className={`text-${
                    message.type === "user" ? "white" : "gray-800"
                  } dark:text-${
                    message.type === "user" ? "gray-200" : "gray-200"
                  }`}
                >
                  {message.text}
                </p>
